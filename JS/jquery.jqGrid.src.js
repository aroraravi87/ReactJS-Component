	...
	return this.each( function() {
		...
		addJSONData = function (data, t, rcnt, more, adjust) {
			...
			while (i<len) {
                ...
                //by ravi add $.unique
				rowReader = UniqueRow(objectReader);
			...
			var triggerToolbar = function() {
				...
				$.each($t.p.colModel,function(){
					...
					if(p.searchOperators ) {
					    ...
					    so = (this.searchoptions && this.searchoptions.sopt) ? 'cn' : this.stype === 'select' ? 'eq' : p.defaultSearch;
					}
	...
	return this.each( function() {
		...
		*/
		...
		this.reDraw = function () {
			...
			if (p.id != "EnvDBGrid")
            {
			    t = this.createTableForGroup(p.filter, null);
            }
		    else
		    {
		        t = this.createEnvTableForGroup(p.filter, null);
			}
			$(this).append(t);
        ...
        var countRules = 0; var selectedClassicValue = "All"; 

        this.createEnvTableForGroup = function(group, parentgroup) {
            ...
            table.append("<tr><td></td><td>Classification</td><td>Column</td><td></td><td></td><td></td></tr>")

            // append rules rows
            if (group.rules !== undefined) {
                var tempTr = null; countRules = 0; //var ruleField = "";
                for (i = 0; i < group.rules.length; i++) {
                    if (group.rules[i].field != "Classification") {
                        countRules++;
                    }
                    else {
                        $(table).find("tr." + countRules + " select.classicTd").val(group.rules[i].data);
                    }
                    tempTr = this.createEnvTableRowForRule(group.rules[i], group);
                    if (tempTr != null) {
                        table.append(
                        tempTr
                        );
		 ...
		 * Create the rule data for the filter
		 */
		this.createEnvTableRowForRule = function(rule, group ) {
		    // save current entity in a variable so that it could
		    // be referenced in anonimous method calls

		    var that=this, $t = getGrid(), tr = $("<tr class="+countRules+"></tr>"),
			//document.createElement("tr"),
		    ...
		    tr.append("<td class='first'></td>");
		    var classicRuleTd = $("<td class='classics'></td>");
		    tr.append(classicRuleTd);


		    // add classification dropdown with previous attribute with rule.field
		    var ruleClassicSelect = $("<select id='classification_" + rule.field + "' previous='" + rule.field + "' class='classicTd'></select>");
		    ruleClassicSelect.append("<option value='All'>All</option><option value='Polymers'>Polymers</option><option value='Discrete Organics'>Discrete Organics</option>"
                + "<option value='Poorly Soluble Discrete Organic'>Poorly Soluble Discrete Organic</option>"
                + "<option value='Metallics'>Metallics</option><option value='UVCBs'>UVCBs</option><select></select>");


		    var _trIndex = parseInt(countRules);
		    _trIndex = _trIndex - 1;
		    //  classification drop down onchange
		    ruleClassicSelect.bind('change', function () {

		        group.rules[_trIndex].clasification = $(this).val();
		        // updated column dropdown attribute with selected value
		        $(this).parents("tr").find(".columnSelect").attr('classicPreviousData', selected);
		    });

		    if (group.rules[_trIndex].clasification == undefined) {
		        group.rules[_trIndex].clasification = "All";
		    }

		    $(ruleClassicSelect).val(group.rules[_trIndex].clasification);

		    classicRuleTd.append(ruleClassicSelect);
		    //alert(selectedClassicValue + " inside"); debugger;
		    $("tr." + countRules + " select.classicTd").val(selectedClassicValue);
		    ...
		    // dropdown for: choosing field
		    var ruleFieldSelect = $("<select classicPreviousData='All' class='columnSelect'></select>"), ina, aoprs = [];
		    ruleFieldTd.append(ruleFieldSelect);

		    // column drop down on change
		    ruleFieldSelect.bind('change',function() {
		        rule.field = $(ruleFieldSelect).val();

		        //var classicValue = $(that).parents("tr").find(".columnSelect").attr('classicPreviousData');
		        //alert(group.rules[_trIndex].field);
		        classicRuleTd.html("");
		        var ruleClassicFieldSelect = $("<select id='classification_" + rule.field + "' previous='" + rule.field + "' class='classicTd'></select>");
		        ruleClassicFieldSelect.append("<option value='All'>All</option><option value='Polymers'>Polymers</option><option value='Discrete Organics'>Discrete Organics</option>"
                + "<option value='Poorly Soluble Discrete Organic'>Poorly Soluble Discrete Organic</option>"
                + "<option value='Metallics'>Metallics</option><option value='UVCBs'>UVCBs</option><select></select>");

		        ruleClassicFieldSelect.bind('change', function () {
		            group.rules[_trIndex].clasification = $(this).val();
		            $(this).parents("tr").find(".columnSelect").attr('classicPreviousData', $(this).val());
		        });
		        ...
		        $(ruleClassicFieldSelect).val(group.rules[_trIndex].clasification);
		        classicRuleTd.append(ruleClassicFieldSelect);
		    ...
		    for (i = 0; i < that.p.columns.length; i++) {
                ...
                ignoreHiding = (that.p.columns[i].searchoptions.searchhidden === true);
		        if ((ignoreHiding && searchable) || (searchable && !hidden)) {
		            selected = ""; if (that.p.columns[i].name != "Classification") {
		                if (rule.field === that.p.columns[i].name) {
		                    selected = " selected='selected'";
		                    j = i;
		                }
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$("#"+fid+"_search").bind('click', function(){
					...
					if ($t.p.id == "EnvDBGrid") {
					    var dataRes = $.grep(filters.rules, function (obj) {
					        return obj.field != "Classification";
					    });
					    ...
					    for (var i = 0; i < dataRes.length; i++) {
					        var coulmnRule = ({
					            field: dataRes[i].field,
					            op: dataRes[i].op,
					            data: dataRes[i].data
					        });

					        filters.rules.push(coulmnRule);
					        var countIndex = i; countIndex = countIndex + 1;
					        var classiFilterRule = ({
					            field: "Classification",
					            op: "eq",
					            data: $("table.group").find("tr." + countIndex + " select.classicTd").val()
					        });

					        filters.rules.push(classiFilterRule);
					    }
					}
...
$.jgrid.extend({
	...
	columnChooser : function(opts) {
        ...
        colModel = UniqueRow(colModel);
        colNames = UniqueRow(colNames);
